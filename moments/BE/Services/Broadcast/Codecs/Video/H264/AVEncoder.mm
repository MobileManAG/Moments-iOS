...
#define OUTPUT_FILE_SWITCH_POINT (10 * 1024 * 1024)  // 5 MB switch point
...
- (void) onParamsCompletion
{
    ...
    if ([self parseParams:_headerWriter.path])
    {
        ...
        while (_inputFile == nil) {
            AVAssetWriterStatus status = _writer.status;
            if (status == AVAssetWriterStatusFailed || status == AVAssetWriterStatusCancelled || status == AVAssetWriterStatusCompleted) {
                break;
            }
        ...
        if (_inputFile == nil) {
            BOOL exists = [[NSFileManager defaultManager] fileExistsAtPath:_writer.path];
            NSLog(@"Can not get file handle: (exists=%d) %@", exists, _writer.path);
        }
...
- (BOOL) encodeFrame:(CMSampleBufferRef) sampleBuffer
{
    ...
    @synchronized(self)
    {
        ...
        success = [_writer encodeFrame:sampleBuffer];
    }
...
- (void) swapFiles:(NSString*) oldPath
{
    ...
    if (_inputFile == nil) {
        AVAssetWriterStatus status = _writer.status;
        BOOL exists = [[NSFileManager defaultManager] fileExistsAtPath:_writer.path];
        NSLog(@"Can not get writer file handle: (status=%d) (file exists=%d) %@", status, exists, _writer.path);
    }
...
- (void) readAndDeliver:(long long) cReady
{
...
- (void) onFileUpdate
{
    ...
    long long cReady = s.st_size - [_inputFile offsetInFile];
    ...
    while (!_foundMDAT && (cReady > 8))
    {
        ...
        if (_bytesToNextAtom > 0)
        {
            long long cThis = cReady < _bytesToNextAtom ? cReady :_bytesToNextAtom;
