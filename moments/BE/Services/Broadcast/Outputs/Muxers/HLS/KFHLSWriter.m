...
- (id) initWithDirectoryPath:(NSString *)directoryPath {
    if (self = [super init]) {
        ...
        _segmentDurationSeconds = [VideoUtils videoSegmentsDuration];
        self.segmentsCount = [VideoUtils videoSegmentsCount];
...
- (void) setupOutputFile {
    self.outputPath = [_directoryPath stringByAppendingPathComponent:VIDEO_INDEX_FILE_NAME];
    
    _outputFile = [[FFOutputFile alloc] initWithPath:self.outputPath options:@{kFFmpegOutputFormatKey: @"hls"}];
...
- (void) addVideoStreamWithWidth:(int)width height:(int)height {
    ...
    av_opt_set_int(_outputFile.formatContext->priv_data, "hls_list_size", self.segmentsCount, 0);
}
...
- (void) processEncodedData:(NSData*)data presentationTimestamp:(CMTime)pts streamIndex:(NSUInteger)streamIndex isKeyFrame:(BOOL)isKeyFrame {
    ...
    dispatch_async(_conversionQueue, ^{
        ...
        //NSLogInfo(@"*** Scaled PTS: %lld", scaledPTS);
        ...
        if (error) {
            ...
            //NSLogVerbose(@"Wrote packet of length %d at streamIndex %d and \t oPTS %lld \t scaledPTS %lld", data.length, streamIndex, originalPTS, scaledPTS);
        }
    });
}
