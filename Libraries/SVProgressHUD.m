...
+ (void)showStatus:(NSString *)string {
    ...
    [[self sharedView] showStatus:string duration:displayInterval maskType:SVProgressHUDDefaultMaskType];
}
...
- (void)updatePosition {
    ...
    CGFloat hudWidth = [UIScreen mainScreen].bounds.size.width;
    CGFloat hudHeight = 64.0f;
...
#pragma clang diagnostic pop
        ...
        CGFloat labelRectY = hudHeight/2-stringHeight/2;
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    self.offsetFromCenter = UIOffsetMake(0, self.frame.size.height/2);
    ...
    CGFloat posY = CGRectGetHeight(orientationFrame)/2;//floor(activeHeight*0.45);
...
- (void)showStatus:(NSString *)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType {
    self.progress = SVProgressHUDUndefinedProgress;
    self.maskType = hudMaskType;
    [self cancelRingLayerAnimation];
